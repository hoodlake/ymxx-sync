...
--添加账户性质列
IF NOT EXISTS(SELECT * FROM SYSCOLUMNS WHERE ID=OBJECT_ID('BT_WARNING_STRATEGY_DETAIL') AND NAME='ACC_CATEGORY')  
ALTER TABLE BT_WARNING_STRATEGY_DETAIL ADD ACC_CATEGORY VARCHAR(20); 
GO
...
IF EXISTS (SELECT * FROM sysobjects WHERE /*检测是否存在*/  
                         name = 'V_BANK_ACC_ALL')  
     DROP VIEW V_BANK_ACC_ALL /*删除视图*/  
GO
CREATE  view V_BANK_ACC_ALL AS
...
IF EXISTS (SELECT * FROM sysobjects WHERE /*检测是否存在*/  
                         name = 'V_CORP_BANKTYPE_NATURE_PROP')  
     DROP VIEW V_CORP_BANKTYPE_NATURE_PROP /*删除视图*/  
GO
CREATE VIEW V_CORP_BANKTYPE_NATURE_PROP AS 
