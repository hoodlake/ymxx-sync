...
create table FINANIC_COST  (
   ...
   LOAD_CORP            VARCHAR2(40),   --放款id
   BILL_NO              VARCHAR2(40),
   BUSS_TIME            DATE,
   COST_TYPE            VARCHAR2(40),
   ...
   UPDATE_TIME          DATE,
   UPDATER              VARCHAR2(40),
   NEXT_CHECKER VARCHAR2(150),     --下一个审批人
   APPROVES VARCHAR2(200),         --当前审批人     
   PASS_DATE DATE,                 --审批通过实践
   CORP_ID NUMBER(10),             --贷款id
...
INSERT INTO BT_SYS_RES (RES_CODE, RES_NAME, SYS_CODE, FATHER_CODE, RES_URL, FUNC_FLAG, RES_TYPE, LINK_TARGET, STATUS, RES_ORDER, RMK, REVERSE1, REVERSE2, REVERSE3, REVERSE4, REVERSE5, REVERSE6, REVERSE7, REVERSE8, REVERSE9, REVERSE10, RES_LEVEL, RES_ROLE)
SELECT (SELECT MAX(T1.RES_CODE) + 1 FROM BT_SYS_RES T1), '费用统计', 'cms', RES_CODE, '/cms/financiCount.do?method=initQuery'||chr(38)||'total=true', '0', '1', '0', '0', (SELECT MAX(T2.RES_ORDER)+1 FROM BT_SYS_RES T2 WHERE T2.FATHER_CODE = T3.RES_CODE AND T2.SYS_CODE = 'cms'), NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 2, '' FROM BT_SYS_RES T3 WHERE T3.RES_NAME = '费用管理' AND T3.SYS_CODE = 'cms';
COMMIT;
