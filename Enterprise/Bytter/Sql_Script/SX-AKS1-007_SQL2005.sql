...
BEGIN
  ...
  BEGIN
   ...
   select(select max(res_code) + 1 from bt_sys_res), '票据贴现', 'cms', 0, '', '0', '1', '0', '0', 8, '', 1,'';
  END;
END;
GO
...
BEGIN
  ...
  BEGIN
   insert into bt_sys_res (RES_CODE, RES_NAME, SYS_CODE, FATHER_CODE, RES_URL, FUNC_FLAG, RES_TYPE, LINK_TARGET, STATUS, RES_ORDER, RMK, RES_LEVEL,RES_ROLE)
   select(select max(res_code) + 1 from bt_sys_res), '贴现申请', 'cms', (select res_code from bt_sys_res  where res_name='票据贴现' and res_level=1), '/AB/bill/discountCms.jsp', '0', '1', '0', '0', 1, '', 2,'';
  END;
END;
GO
...
BEGIN
  ...
  BEGIN
   insert into bt_sys_res (RES_CODE, RES_NAME, SYS_CODE, FATHER_CODE, RES_URL, FUNC_FLAG, RES_TYPE, LINK_TARGET, STATUS, RES_ORDER, RMK, RES_LEVEL,RES_ROLE)
   select(select max(res_code) + 1 from bt_sys_res), '贴现补录', 'cms', (select res_code from bt_sys_res  where res_name='票据贴现' and res_level=1), '/AB/discountManage.do?method=loadlist'+char(38)+'operationType=4'+char(38)+'discountCms=true', '0', '1', '0', '0', 1, '', 2,'';
  END;
END;
GO
