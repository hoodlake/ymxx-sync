...
--修改内容：增加字段
GO
ALTER TABLE bis_acc_his_dtl ADD TRANS_TYPE VARCHAR(10);
ALTER TABLE bis_acc_his_dtl ADD TRANS_STATUS CHAR(1);
ALTER TABLE bis_acc_his_dtl ADD RETURN_MSG VARCHAR(100);
GO
ALTER TABLE bis_acc_his_dtl_mergen ADD TRANS_TYPE VARCHAR(10);
ALTER TABLE bis_acc_his_dtl_mergen ADD TRANS_STATUS CHAR(1);
ALTER TABLE bis_acc_his_dtl_mergen ADD RETURN_MSG VARCHAR(100);
GO
ALTER TABLE bis_acc_his_dtl_split ADD TRANS_TYPE VARCHAR(10);
ALTER TABLE bis_acc_his_dtl_split ADD TRANS_STATUS CHAR(1);
ALTER TABLE bis_acc_his_dtl_split ADD RETURN_MSG VARCHAR(100);
GO
ALTER TABLE ab_operation ADD TRANS_TYPE VARCHAR(10);
ALTER TABLE ab_operation ADD TRANS_STATUS CHAR(1);
ALTER TABLE ab_operation ADD RETURN_MSG VARCHAR(100);
GO
--增加“发送次数”列
alter table ab_operation add send_frequency int default 0;
alter table BIS_ACC_HIS_DTL_SPLIT add send_frequency int default 0;
alter table bis_acc_his_dtl_mergen add send_frequency int default 0;
alter table bis_acc_his_dtl add send_frequency int default 0;
go
--ERP凭证号
alter table BIS_ACC_HIS_DTL add ERP_VOUCHER_NO VARCHAR(30);
alter table AB_OPERATION add ERP_VOUCHER_NO VARCHAR(30);
go
...
select convert(varchar(20),split.id) id,
...
union all
select convert(varchar(20),mergen.id) id,
       '' as serial_id,
...
select one.serial_id id,
      ...
      convert(varchar(20),one.corp_id)  corp_id,--单位Id
