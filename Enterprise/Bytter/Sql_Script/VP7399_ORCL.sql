--凭证模板定义主表新增“凭证类型”
alter table erp_voucher_template add voucher_type varchar2(120);
--凭证模板定义主表新增“业务范围”
alter table erp_voucher_template add business_area varchar2(60);

comment on column erp_voucher_template.voucher_type is '凭证类型(多个用英文分号隔开)';
comment on column erp_voucher_template.business_area is '业务范围';   

--历史明细表新增“凭证类型”
alter table bis_acc_his_dtl add voucher_type varchar2(60);
--历史明细表新增“业务范围”
alter table bis_acc_his_dtl add business_area varchar2(60);

comment on column bis_acc_his_dtl.voucher_type is '凭证类型';
comment on column bis_acc_his_dtl.business_area is '业务范围';


--历史明细合并表新增“凭证类型”
alter table bis_acc_his_dtl_mergen add voucher_type varchar2(60);
--历史明细合并表新增“业务范围”
alter table bis_acc_his_dtl_mergen add business_area varchar2(60);

comment on column bis_acc_his_dtl_mergen.voucher_type is '凭证类型';
comment on column bis_acc_his_dtl_mergen.business_area is '业务范围';


--历史明细拆分表新增“凭证类型”
alter table bis_acc_his_dtl_split add voucher_type varchar2(60);
--历史明细拆分表新增“业务范围”
alter table bis_acc_his_dtl_split add business_area varchar2(60);

comment on column bis_acc_his_dtl_split.voucher_type is '凭证类型';
comment on column bis_acc_his_dtl_split.business_area is '业务范围';
...
select dtl.serial_id serial_id,
  ...
  left join bt_currency currency on dtl.cur = currency.cur_code, v_acc_list acc,
 bt_corp corp
